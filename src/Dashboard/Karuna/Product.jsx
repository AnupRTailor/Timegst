...
function Product() {
    ...
    const [selectedCompany, setSelectedCompany] =  useState("KARUNA ENTERPRISE");
    ...
    const fetchData = () => {
        fetch('http://localhost/TImegst/react-gst/backend/api/karuna/product.php')
            .then(response => {
                if (!response.ok) {
    ...
    const handleCompanyChange = (e) => {
        ...
        if (e.target.value === "EFSOULS LIMITED LIABILITY PARTNERSHIP") {
    ...
    return (
        <div>
            ...
            <div className="action-buttons">
                <button style={{ marginLeft: '260px'}} className="add-product-button" onClick={(e) => (e.ctrlKey ? window.open('/Dashboard/Karuna/Addproduct', '_blank') : window.location.href = '/Dashboard/Karuna/Addproduct')}>
                    Add Product
                </button>
                ...
                <tbody>
                    {currentProducts.map(product => (
                        <tr key={product.id}>
                            ...
                            <td>
                                <a href= {`/Dashboard/Karuna/EditProduct/${product.id}`} >
